...
The bash gate refuses the mutating cloud and cluster commands as well: the `kubectl` write verbs (`delete`, `apply`, `create`, `replace`, `edit`, `patch`, `scale`, `rollout`, `exec`, `cordon`, `drain`, `label`, `annotate`, `expose`, `run`, `taint`, `cp`, `attach`, `port-forward`, `proxy`, and `set` by subcommand); `helm upgrade`, `install`, `uninstall` and `rollback`; `terraform apply`, `destroy`, `import` and `state`; the AWS CLI's 37 mutating verb families, written as `*aws *<verb>-*` so a service the CLI adds later is covered without a new rule; the writing `aws s3` verbs; `aws configure set`, `aws ssm start-session` and `aws ecs execute-command`; and the mutating calls whose verb carries no hyphen, such as `lambda invoke`, `sns publish` and `cloudformation deploy`. Every pattern tolerates flags before the verb and an environment prefix in front of the command, which is how these are typed in practice; an anchored pattern alone allowed `kubectl -n prod delete pod x`.

The same patterns refuse some reads, since each is matched against the whole command string: a read whose own verb is a mutating word, such as `aws logs stop-query`, `aws logs start-live-tail` and `aws cloudtrail cancel-query`, and any allowed read whose arguments carry one. Over-denial is the safe direction for a deny rule. Four reads are allowed back after the deny block, because for their data the denied verb is the only read path: `aws logs start-query` and `stop-query`, and `aws cloudtrail start-query` and `cancel-query`. The re-allows are narrow and anchored while the denies are wide, on purpose.

The list is a speed bump and not a boundary: an interpreter such as `sh -c` or `python -c`, a script the agent writes and then runs, or a denied command chained after an allowed one, is a command string that no pattern here describes. It stops a careless step. What holds is the role granted to the machine the agent runs on.
