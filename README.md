...
Before opencode starts, both doors also remove four things from the checkout: `opencode.json` at the root, `.opencode/opencode.json`, `.opencode/plugin` and `.opencode/command`. opencode merges what the package passes in with the repository's own configuration rather than replacing it, and that merge keeps the repository's key order while a permission resolves to the last rule that matches, so a tracked `opencode.json` naming the same keys can hold the package's denies at its own positions and let its own catch-all win. A file under `.opencode/plugin` does not need any of that: opencode imports it, so it runs before a gate is consulted at all. A tracked file is marked `skip-worktree` first, so no pull request carries its removal, and nothing in your repository on GitHub changes. `.opencode/agents` is left alone, because that is where the package writes its own agent.
...
- The bash gate refuses `git push`, `git remote`, `curl`, `wget`, `ssh`, `scp`, `rm -rf` and `sudo`, each written so that an environment prefix (`HTTPS_PROXY=... curl`), a wrapper (`env curl`), an absolute path (`/usr/bin/curl`) and a flag between the binary and the verb (`git -C /tmp/r push`) do not walk past it. opencode parses the command line and applies the patterns to each command in it, denying the call when any one of them is denied, so a refused command does not get through by being chained after an allowed one either. What a pattern sees is that command as written, arguments included.
